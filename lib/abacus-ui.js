Raphael(function() {
	...
	function updateClock(date) {
		...
		for(var i = 0; i < 8; i++) {
			...
			if(d !== 0) { // is here a change (stones going to the border line or back)
	...
	// function that repeatedly tries to update the clock ui
	// waits always half of the time till the next full minute
	function tryUpdate() {
		var now = new Date(),
			waitMillis = 60000 - (now.getSeconds() * 1000 + now.getMilliseconds());
		
		updateClock(now);
		
		setTimeout(tryUpdate, Math.max(waitMillis / 2, 50));
	}
